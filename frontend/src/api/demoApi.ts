...
/**
 * Updates worker pool size directly on processor-service (Port 8082).
 */
export async function updateProcessorWorkers(
  workers: number
): Promise<ProcessorDirectStatus> {
  const response = await fetch(`${PROCESSOR_SERVICE_URL}/processor/workers`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  if (!response.ok) {
    throw new Error(`Failed to update processor workers: ${response.statusText}`);
  }
  return response.json();
}
